 ...
 *   /           → default
 *   /projects/  → projects
 *   /projects/sto-info/ → projects-sto-info
 *
...
async function generateOgImage({ title, description, logoDataUrl, titleFont, bodyFont }) {
  ...
  const node = el(
    ...
    // Full-bleed background — site palette (no hero image on this site)
    el("div", {
      style: {
        position: "absolute",
        top: "0px",
    ...
    el("div", {
      style: {
        ...
        height: "630px",
        background:
          "linear-gradient(to left, rgba(37,99,235,0.35) 0%, rgba(37,99,235,0.12) 35%, rgba(11,18,32,0) 65%)",
      },
    }),
    // Content row
    el(
  ...
  const svg = await satori(node, {
    ...
    height: 630,
    fonts: [
      { name: "Inter", data: titleFont, weight: 700, style: "normal" },
